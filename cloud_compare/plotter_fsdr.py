...
def generate_x_values(num_points, increment=30):
...
def main():
    ...
    parser.add_argument('--increment', type=int, default=30, help='Increment value for x-axis')
    ...
    folder_path = os.path.join(os.path.dirname(__file__), "results FSDR")
    plotter_folder = os.path.join(folder_path, "plotter FSDR")
    os.makedirs(plotter_folder, exist_ok=True)  # Create "plotter" subdirectory if it doesn't exist

    # Locate the file "output f.txt"
    file_path = os.path.join(folder_path, "output FSDR.txt")
    ...
    if os.path.isfile(file_path):
        ...
        plot_filename = os.path.join(plotter_folder, f'plot_FSDR_{today}.png')
        plt.savefig(plot_filename)
        print(f'Plot saved as: {plot_filename}')

        # Write the coefficients of the polynomial fit to a text file
        polyfit_filename = os.path.join(plotter_folder, 'polynomial FSDR.txt')
        ...
        print("File 'output FSDR.txt' not found in 'results FSDR' folder.")
