...
from lib.common.myBNI27 import BNI                                                                                  #USE MODIFIED BNI --> myBNI27
...
if __name__ == "__main__":
    ...
    for data in pbar:
        ...
        def write_smpl_info_to_txt(smpl_info, file_path):
            with open(file_path, 'w') as file:
             for key, value in smpl_info.items():
                 if isinstance(value, torch.Tensor):
                    value = value.tolist()  # Convert tensor to a nested Python list
        ...
        for idx in range(N_body):
            ...
            txt_path = f"{args.out_dir}/{cfg.name}/obj/{data['name']}_smpl_{idx:02d}.txt"
            kp_path = f"{args.out_dir}/{cfg.name}/obj/{data['name']}_kp_{idx:02d}.txt"
            ...
            if not osp.exists(smpl_obj_path):
                ...
                smpl_info = {
                    ...
                    "bodypose_mat":
                    optimed_pose_mat[idx].detach().cpu().unsqueeze(0),
                    "global_orient":
                    ...
                    data["scale"][idx].cpu(),
                    "smpl_joints":
                    in_tensor["smpl_joint"][idx].detach().cpu(),
                }
                keypoints ={
                    "smpl_joints":
                    in_tensor["smpl_joint"][idx].detach().cpu(),
                ...
                write_smpl_info_to_txt(smpl_info, txt_path)
                write_smpl_info_to_txt(keypoints, kp_path)
        ...
        for idx in range(N_body):
            ...
            # save normals, depths and masks                                                                                 #important step in BNI object creation
            ...
            # BNI process                                                                                                        #creates BNI object
            BNI_object = BNI(
            ...
            if cfg.bni.use_ifnet:
                ...
                in_tensor.update(
                    dataset.depth_to_voxel({
                        "depth_F": BNI_object.F_depth.unsqueeze(0), "depth_B":                                       #takes front and back depth maps,adds a batch dimension to them,  
                        BNI_object.B_depth.unsqueeze(0)                                                              #and then passes them to the voxelization function, results saved in in_tensor
                    })
                )
            ...
            BNI_object.F_trimesh.export(                                                                                            #EXPORT FNM
                f"{args.out_dir}/{cfg.name}/obj/{data['name']}_{idx}_Front_BNI.obj"                                                 #EXPORT FNM
            )
